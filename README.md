...
- **Dresser Upgrade Advisor:** Automatically scans your stored items when you change jobs in a Sanctuary and notifies you in chat if it finds a smart gear upgrade based on Item Level and Stat Priorities.
...
### Dresser Upgrade Advisor (Smart Evaluation)

Turn your Glamour Dresser into a secondary armory! When you change jobs in a Sanctuary (where Dressers are usually located), the plugin automatically checks if any stored gear would be an upgrade over your currently equipped items.
- **Item Level is King:** The primary deciding factor is always the Item Level (iLvl).
- **Secondary Stat Tiebreaker:** When comparing two items of the exact same iLvl, the plugin uses a built-in stat priority system to determine the winner based on your Job's Role (e.g. Healers prioritize Critical Hit over Piety).
- **Job Specific:** It correctly ensures the recommended item provides the necessary Main Stat for your job (Strength, Dexterity, Intelligence, or Mind) and ignores pure glamour items (iLvl 1).
