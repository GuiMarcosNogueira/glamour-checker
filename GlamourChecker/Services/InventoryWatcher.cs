...
public unsafe class InventoryWatcher {
    ...
    public void ScanDresserAndArmoire() {
        ...
        // Armoire
        if (_memoryProvider.IsCabinetLoaded()) {
        ...
        if (dresserItems.Length > 0) {
            ...
            updated = true;
            // Services.PluginLog.Info($"[GlamourChecker] Forcing Dresser update: {currentItemCount} total items.");
        }
    ...
    public List<InventoryItemInfo> GetUnstoredItemsInBags() {
        ...
        foreach (var type in typesToCheck) {
            ...
            for (int i = 0; i < items.Length; i++) {
                var item = items[i];
                if (item.ItemId != 0) {
                    var actualItemId = item.ItemId;
                    if (actualItemId > 1000000) {
                        actualItemId -= 1000000;
                    }
                    var modelId = _modelScanner.GetModelId(actualItemId);
    ...
    public List<DuplicateAppearance> GetDuplicates() {
        var duplicates = new List<DuplicateAppearance>();
        var combined = new Dictionary<ulong, List<uint>>();
        
        if (_config.DresserItemsByModel != null) {
            foreach (var kvp in _config.DresserItemsByModel) {
                if (!combined.ContainsKey(kvp.Key)) combined[kvp.Key] = new List<uint>();
                combined[kvp.Key].AddRange(kvp.Value);
            }
        }
        if (_config.ArmoireItemsByModel != null) {
            foreach (var kvp in _config.ArmoireItemsByModel) {
                if (!combined.ContainsKey(kvp.Key)) combined[kvp.Key] = new List<uint>();
                combined[kvp.Key].AddRange(kvp.Value);
            }
        }
        ...
        foreach (var kvp in combined) {
            if (kvp.Value.Count > 1) {
                duplicates.Add(new DuplicateAppearance {
                    ModelId = kvp.Key,
                    ItemIds = kvp.Value
                });
            }
